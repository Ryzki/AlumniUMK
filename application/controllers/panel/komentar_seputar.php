...
class Komentar_seputar extends CI_Controller{
	...
	public function index(){
		if($this->session->userdata('logged_in_umk')) {
			$data['daftar_komentar'] = $this->komentar_model->select_seputar_all()->result();
			$this->template->display('panel/komentarseputar_v', $data);
		} else {
	...
	public function deletedata($kode) {
		...
		if ($kode == null) {
			...
			$this->komentar_model->delete_data_seputar($kode);
			echo "<meta http-equiv=refresh content=0;url=\"".site_url()."panel/komentar_seputar\">";
		}
	}
}
/* Location: ./application/controllers/panel/komentar_seputar.php */
?>